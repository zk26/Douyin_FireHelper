...
(function () {
  ...
  class Utils {
    ...
    static esc(str) { const el = document.createElement('span'); el.textContent = str; return el.innerHTML; }
    static httpGet(url, { timeout = 10000, responseType = 'text' } = {}) {
      return new Promise((ok, fail) => {
        ...
        GM_xmlhttpRequest({ method: 'GET', url, responseType,
          onload(r) { clearTimeout(t); r.status >= 200 && r.status < 300 ? ok(responseType === 'json' ? r.response : r.responseText) : fail(new Error(`HTTP ${r.status}`)); },
          onerror() { clearTimeout(t); fail(new Error('网络错误')); },
          ontimeout() { clearTimeout(t); fail(new Error('请求超时')); },
        });
      });
    }
    static httpPost(url, data, { timeout = 5000 } = {}) {
      return new Promise((ok, fail) => {
        const t = setTimeout(() => fail(new Error('请求超时')), timeout);
        GM_xmlhttpRequest({ method: 'POST', url, headers: { 'Content-Type': 'application/json' }, data: JSON.stringify(data),
          onload(r) { clearTimeout(t); ok(r); },
          onerror() { clearTimeout(t); fail(new Error('网络错误')); },
          ontimeout() { clearTimeout(t); fail(new Error('请求超时')); },
        });
      });
  ...
  class Logger {
    ...
    #bus; #minLevel;
    constructor(bus, minLevel = 'DEBUG') { this.#bus = bus; this.#minLevel = Logger.LEVELS[minLevel] || 0; }
    #log(lv, msg) {
  ...
  class Config {
    ...
    static DEF = Object.freeze({
      configVersion: 4,
      baseMessage: '续火', sendTime: '00:01:00', sendTimeRandom: false,
      sendTimeRangeStart: '23:30:00', sendTimeRangeEnd: '00:30:00',
      maxRetryCount: 3, targetUsernames: '', userSearchTimeout: 15000,
      maxHistoryLogs: 300, pageLoadWaitTime: 5000, multiUserMode: 'sequential',
      fireDays: 1, lastFireDate: '', customMessage: '续火 | 火花已续 [天数] 天',
      reigniteMessage: '续火 | 重燃中 [重燃进度]',
      expiringMessage: '续火 | 火花即将消失 [剩余天数]',
      autoRetryInterval: 10, enableCallback: false, callbackPort: 7788,
      initialDelay: 30, theme: 'dark', panelTop: null, panelLeft: null, panelRight: null,
      autoSendEnabled: true,
    });
    #s; #l; #d = {};
    constructor(s, l) { this.#s = s; this.#l = l; this.load(); }
    ...
    load() {
      ...
      const sd = this.#s.get('fireDays', null); if (sd !== null) this.#d.fireDays = sd;
      const ld = this.#s.get('lastFireDate', null); if (ld !== null) this.#d.lastFireDate = ld;
    }
    save() { this.#s.set('userConfig', this.#d); this.#s.set('fireDays', this.#d.fireDays); this.#s.set('lastFireDate', this.#d.lastFireDate); }
    update(p) { Object.assign(this.#d, p); this.save(); }
  ...
  class DOM {
    #sel; #log;
    constructor(sel, log) { this.#sel = sel; this.#log = log; }
    ...
    wait(sel, timeout = 10000, parent = document) {
      return new Promise(resolve => {
        ...
        let settled = false;
        const done = (v) => { if (settled) return; settled = true; obs.disconnect(); clearInterval(poller); clearTimeout(t); resolve(v); };
    ...
    shiftEnter(el) { ['keydown', 'keypress', 'keyup'].forEach(t => this.keyEvent(el, t, 'Enter', { shiftKey: true })); }
    ...
    async inputMsg(editor, message) {
      ...
      // 和 V1 完全一致的流程
      editor.focus(); this.click(editor); await Utils.sleep(30);
      ...
      for (let li = 0; li < lines.length; li++) {
        for (let ci = 0; ci < lines[li].length; ci++) {
          ...
          if (!ok) {
            ...
            let range = sel.rangeCount > 0 && editor.contains(sel.anchorNode)
              ? sel.getRangeAt(0) : (() => { const r = document.createRange(); r.selectNodeContents(editor); r.collapse(false); sel.removeAllRanges(); sel.addRange(r); return r; })();
        ...
        if (li < lines.length - 1) { await Utils.sleep(30); this.shiftEnter(editor); await Utils.sleep(50); }
      }
    ...
    setReactVal(input, value) {
      ...
      const last = input.value; setter.call(input, value);
      const tracker = input._valueTracker; if (tracker) tracker.setValue(last);
    ...
    async typeChars(input, text) {
      ...
      for (const ch of text) {
        ...
        const tracker = input._valueTracker; if (tracker) tracker.setValue(cur.slice(0, -1));
  ...
  // ============================================================
  class FSM {
    #bus; #log; #cur = 'Idle'; #ac = null;
    constructor(bus, l) { this.#bus = bus; this.#log = l; }
    static T = Object.freeze({
      Idle: ['SearchingUser'], SearchingUser: ['SendMessage', 'Idle'],
      SendMessage: ['Success', 'Idle'], Success: ['SearchingUser', 'Idle'],
    });
    ...
    go(to) {
      ...
      this.#cancelTimeout(); const from = this.#cur; this.#cur = to;
      this.#log.debug(`状态: ${from} → ${to}`); this.#bus.emit('state:change', { from, to }); return true;
    }
    reset() { this.#cancelTimeout(); const p = this.#cur; this.#cur = 'Idle'; if (p !== 'Idle') this.#bus.emit('state:change', { from: p, to: 'Idle' }); }
    forceIdle() { this.#cancelTimeout(); const p = this.#cur; this.#cur = 'Idle'; if (p !== 'Idle') this.#bus.emit('state:change', { from: p, to: 'Idle' }); }
    #cancelTimeout() { if (this.#ac) { this.#ac.abort(); this.#ac = null; } }
  }
  ...
  class SearchService {
    #app; #dom; #log;
    constructor(app) { this.#app = app; this.#dom = app.dom; this.#log = app.logger; }
    ...
    async search(username) {
      ...
      try {
        ...
        if (!btn) {
          ...
          if (item) { this.#dom.click(item); }
          else { this.#dom.setReactVal(si, ''); return { ok: false, error: '未找到聊天按钮' }; }
        } else {
          btn.focus(); await Utils.sleep(50); this.#dom.click(btn);
        }
  ...
  class ChatService {
    #dom; #sel;
    constructor(app) { this.#dom = app.dom; this.#sel = app.selector; }
    ...
    async scrollTop() {
      ...
      if (list) { await Utils.sleep(300); list.scrollTop = 0; }
    }
  ...
  class SenderService {
    ...
    async send(username) {
      ...
      try { msg = await this.#app.messageService.build(username); } catch (e) {
        this.#log.error(`消息构建失败: ${e.message}`); msg = `${this.#app.config.data.baseMessage} | 获取失败~`;
      }
      const cleanMsg = msg.replace(/^[\s\u3000\uFEFF\xA0\n\r]+/, '').replace(/[\s\u3000\uFEFF\xA0\n\r]+$/, '');

      let editor = await this.#app.chatService.findEditor(10000);
      if (!editor) { await Utils.sleep(2000); editor = await this.#app.chatService.findEditor(5000); }
  ...
  // ============================================================
  class RetryService {
    #s; #l; #m = new Map(); #f = new Set();
    constructor(s, l) { this.#s = s; this.#l = l; const sv = this.#s.get('retryMap', {}); this.#m = new Map(Object.entries(sv)); this.#f = new Set(this.#s.get('failedToday', [])); }
    #save() { this.#s.set('retryMap', Object.fromEntries(this.#m)); this.#s.set('failedToday', [...this.#f]); }
    cnt(u) { return this.#m.get(u) || 0; }
    failed(u) { return this.#f.has(u); }
    canRetry(u, max) { return !this.#f.has(u) && this.cnt(u) < max; }
    inc(u) { this.#m.set(u, this.cnt(u) + 1); this.#save(); }
    markFail(u) { this.#f.add(u); this.#save(); this.#l.warn(`${u} 耗尽重试`); }
    reset() { this.#m.clear(); this.#f.clear(); this.#save(); }
  ...
  class CallbackService {
    #app; #log;
    constructor(app) { this.#app = app; this.#log = app.logger; }
    async notify(payload) {
      const c = this.#app.config.data; if (!c.enableCallback) return;
      try { await Utils.httpPost(`http://localhost:${c.callbackPort || 7788}/done`, { ...payload, ts: Date.now() }); this.#log.info('已通知后端'); } catch (e) { this.#log.error(`回调失败: ${e.message}`); }
    }
  }

  class StatsService {
    #s; #l; sent = []; #d = '';
    constructor(s, l) { this.#s = s; this.#l = l; this.#d = this.#s.get('statsDate', ''); const t = Utils.today(); if (this.#d !== t) { this.sent = []; this.#d = t; this.#save(); } else { this.sent = this.#s.get('sentToday', []); } }
    #save() { this.#s.set('sentToday', this.sent); this.#s.set('statsDate', this.#d); }
    mark(u) { if (!this.sent.includes(u)) { this.sent.push(u); this.#save(); } }
    ...
    reset() { this.sent = []; this.#d = Utils.today(); this.#save(); }
    get date() { return this.#d; }
  }

  class HistoryService {
    #s; #l; #max; #queue = []; #flushTimer = null;
    constructor(s, l, max = 300) { this.#s = s; this.#l = l; this.#max = max; }
    add(msg, level = 'INFO') {
      // P2: 批量写入 - 先缓存，2秒后统一写
    ...
    clear() { this.#queue = []; this.#s.set('historyLogs', []); this.#l.info('历史日志已清空'); }
    export() { const logs = this.all(); const t = logs.map(l => `${new Date(l.ts).toLocaleString()} [${l.level}] ${l.msg}`).join('\n'); Utils.download(`续火日志_${Utils.today()}.txt`, t); this.#l.info('日志已导出'); }
  }
  ...
  class Scheduler {
    #app; #cfg; #log;
    #autoTmr = null; #cdTmr = null; #next = null; #ready = false;
    constructor(app) { this.#app = app; this.#cfg = app.config; this.#log = app.logger; }
    get nextTime() { return this.#next; }
    start() {
      const d = this.#cfg.data.initialDelay || 0;
      if (d > 0) { this.#log.info(`等待 ${d}s`); setTimeout(() => { this.#ready = true; this.#log.info('初始延迟结束'); this.#startAuto(); }, d * 1000); }
      else { this.#ready = true; this.#startAuto(); }
      this.#next = this.#calcNext(); this.#startCD(this.#next);
    }
    stop() { clearInterval(this.#autoTmr); clearInterval(this.#cdTmr); this.#autoTmr = this.#cdTmr = null; }
    refresh() { this.#next = this.#calcNext(); this.#startCD(this.#next); }
    check() { this.#check(); }
    #startAuto() { clearInterval(this.#autoTmr); this.#autoTmr = setInterval(() => this.#check(), 15000); this.#check(); }
    #check() {
      if (!this.#ready || this.#app.paused || !this.#app.autoSendEnabled || this.#app.fsm.state !== 'Idle') return;
      const t = Utils.today();
      if (this.#app.stats.date !== t) { this.#log.info('新的一天'); this.#app.resetDay(); }
      if (this.#app.config.hasTargetUsers) { if (this.#app.parsedUsers.every(u => this.#app.stats.isSent(u) || this.#app.retry.failed(u))) { this.#app.checkDone(); return; } }
      else if (this.#app.stats.count > 0) return;
      if (this.#shouldSend()) { this.#log.info('发送时间到'); this.#app.startSend(); }
    }
    ...
    #inRange(a, b) { const { h: sh, m: sm } = Utils.tp(a); const { h: eh, m: em } = Utils.tp(b); const n = new Date().getHours() * 60 + new Date().getMinutes(); const s = sh * 60 + sm, e = eh * 60 + em; return e > s ? (n >= s && n <= e) : (n >= s || n <= e); }
    #calcNext() {
      const c = this.#cfg.data; const now = new Date();
      if (c.sendTimeRandom) { const { h: sh, m: sm } = Utils.tp(c.sendTimeRangeStart); const { h: eh, m: em } = Utils.tp(c.sendTimeRangeEnd); const sM = sh * 60 + sm, eM = eh * 60 + em; let rM = eM > sM ? sM + Math.floor(Math.random() * (eM - sM)) : sM + Math.floor(Math.random() * (1440 - sM + eM)); const t = new Date(now); t.setHours(Math.floor(rM / 60) % 24, rM % 60, 0, 0); if (t <= now) t.setDate(t.getDate() + 1); return t; }
      const { h, m, s } = Utils.tp(c.sendTime); const t = new Date(now); t.setHours(h, m, s, 0); if (t <= now) t.setDate(t.getDate() + 1); return t;
    }
    #startCD(target) {
      clearInterval(this.#cdTmr);
      const fn = () => { const diff = target - new Date(); if (diff <= 0) { this.#app.ui.setCD('00:00:00'); clearInterval(this.#cdTmr); this.#check(); this.#next = this.#calcNext(); this.#startCD(this.#next); return; } const hh = Math.floor(diff / 3600000), mm = Math.floor((diff % 3600000) / 60000), ss = Math.floor((diff % 60000) / 1000); this.#app.ui.setCD(`${String(hh).padStart(2, '0')}:${String(mm).padStart(2, '0')}:${String(ss).padStart(2, '0')}`); }; fn(); this.#cdTmr = setInterval(fn, 1000);
    }
  }
  ...
  // ============================================================
  class UI {
    #app; #panel = null; #reopen = null;
    static #dragging = false; static #dragEl = null; static #ox = 0; static #oy = 0; static #dragBound = false;

    constructor(app) { this.#app = app; }
    get #log() { return this.#app.logger; }

    create() { this.#injectCSS(); this.#mainPanel(); this.#reopenBtn(); this.#log.info('UI 已创建'); }
    show() { if (this.#panel) { this.#panel.style.display = ''; this.#reopen.style.display = 'none'; } }
    hide() { if (this.#panel) { this.#panel.style.display = 'none'; this.#reopen.style.display = 'flex'; } }

    setStatus(t, c) { const el = document.getElementById('dfh-st'); if (el) { el.textContent = t; if (c) el.style.color = c; } }
    setUserStatus(t, ok) { const el = document.getElementById('dfh-ust'); if (!el) return; el.textContent = t; el.style.color = ok === true ? '#30d158' : ok === false ? '#ff453a' : '#8e8e93'; }
    ...
    setPauseBtn(paused) {
      const btn = document.getElementById('dfh-pb'); if (!btn) return;
      btn.innerHTML = paused ? '▶ 今日继续' : '⏸ 今日暂停';
      btn.className = paused ? 'dfh-btn dfh-btn-g' : 'dfh-btn dfh-btn-o';
      const badge = document.getElementById('dfh-pbadge'); if (badge) badge.style.display = paused ? '' : 'none';
    }
    setAutoSendBtn(enabled) {
      const btn = document.getElementById('dfh-asb'); if (!btn) return;
      btn.innerHTML = enabled ? '🔔 定时开' : '🔕 定时关';
      btn.className = enabled ? 'dfh-btn dfh-btn-b' : 'dfh-btn';
      const badge = document.getElementById('dfh-asbadge'); if (badge) badge.style.display = enabled ? 'none' : '';
    }
    addLog(e) { const c = document.getElementById('dfh-lg'); if (!c) return; const d = document.createElement('div'); d.className = 'dfh-le'; d.innerHTML = `<span class="dfh-lt">${e.time}</span><span class="dfh-lm" style="color:${Logger.COLORS[e.level]||'#ff9f0a'}">${e.msg}</span>`; c.prepend(d); while (c.children.length > 30) c.removeChild(c.lastChild); }
    updateProg() { const all = this.#app.parsedUsers; const s = this.#app.stats.count; const t = all.length; this.setProgress(`${s} / ${t}`); this.setStatus(s >= t && t > 0 ? '已完成' : t > 0 ? `进行中` : '未发送', s >= t && t > 0 ? '#30d158' : '#ff453a'); }
...
`;
    ...
    #mainPanel() {
      const old = document.getElementById('dfh-panel'); if (old) old.remove();
      const c = this.#app.config.data;
      const p = document.createElement('div');
      p.id = 'dfh-panel'; p.className = 'dfh-win';
      p.style.width = '380px';
      const savedTop = c.panelTop, savedLeft = c.panelLeft;
      if (savedTop != null && savedLeft != null) { p.style.top = savedTop + 'px'; p.style.left = savedLeft + 'px'; }
...
<div class="dfh-log-w">
  ...
  <div id="dfh-lg" class="dfh-log"><div class="dfh-le"><span class="dfh-lt">${new Date().toLocaleTimeString()}</span><span class="dfh-lm" style="color:#30d158">系统已就绪</span></div></div>
      ...
      p.querySelector('#dfh-rst').onclick = () => { this.#app.stats.reset(); this.#app.retry.reset(); this.#app.messageService.resetDay(); this.updateProg(); this.#log.info('记录已重置'); };
    ...
    static #initDrag() {
      ...
      document.addEventListener('mousedown', e => {
        ...
        if (!win) return;
        UI.#dragging = true; UI.#dragEl = win;
        const r = win.getBoundingClientRect(); UI.#ox = e.clientX - r.left; UI.#oy = e.clientY - r.top;
        win.style.transition = 'none'; document.body.style.userSelect = 'none'; e.preventDefault();
      }, true);
      document.addEventListener('mousemove', e => {
        ...
        UI.#dragEl.style.left = x + 'px'; UI.#dragEl.style.top = y + 'px'; UI.#dragEl.style.right = 'auto'; UI.#dragEl.style.transform = 'none';
      }, true);
      document.addEventListener('mouseup', () => {
        if (!UI.#dragging) return; UI.#dragging = false;
        ...
        UI.#dragEl = null; document.body.style.userSelect = '';
      }, true);
    }

    #settings() {
      const old = document.getElementById('dfh-cfg-p'); if (old) { old.remove(); return; }
      const c = this.#app.config.data;
      const p = document.createElement('div'); p.id = 'dfh-cfg-p'; p.className = 'dfh-ov';
...
<div class="dfh-ov-foot"><button id="dfh-save" class="dfh-btn dfh-btn-b" style="width:100%;padding:10px;font-size:13px">💾 保存设置</button></div>`;
      ...
      const navs = p.querySelectorAll('.dfh-ni'), tabs = p.querySelectorAll('.dfh-tab');
      navs.forEach(n => n.onclick = () => { navs.forEach(x => x.classList.remove('ac')); n.classList.add('ac'); tabs.forEach(t => t.style.display = t.dataset.tab === n.dataset.tab ? '' : 'none'); });
      p.querySelector('#dfh-x-tr').onchange = e => { p.querySelector('#dfh-x-ft').style.display = e.target.checked ? 'none' : ''; p.querySelector('#dfh-x-rt').style.display = e.target.checked ? '' : 'none'; };
    ...
    #history() {
      const old = document.getElementById('dfh-his-p'); if (old) { old.remove(); return; }
      const logs = this.#app.hist.all();
      const p = document.createElement('div'); p.id = 'dfh-his-p'; p.className = 'dfh-ov';
